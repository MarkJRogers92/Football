...
(async()=>{
 ...
 for(const [label,viewport] of [['desktop',{width:1280,height:900}],['iphone',{width:390,height:844}]]){
  ...
  await page.click('#rwSwitch [data-rwm="table"]');await page.waitForFunction(()=>!document.querySelector('#recruiting > .table-wrap')?.classList.contains('recruit-table-hidden'));const tableState=await page.evaluate(()=>({workspaceHidden:!document.querySelector('#recruitWorkspace')?.offsetParent,visibleRows:[...document.querySelectorAll('#recruitBody tr')].filter(r=>r.getClientRects().length&&getComputedStyle(r).display!=='none').length,tableHidden:document.querySelector('#recruiting > .table-wrap')?.classList.contains('recruit-table-hidden')}));check(`[${label}] Table mode restores dense legacy board`,!tableState.tableHidden&&tableState.workspaceHidden&&tableState.visibleRows>0,JSON.stringify(tableState));
