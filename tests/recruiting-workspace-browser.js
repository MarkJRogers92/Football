...
(async()=>{
 ...
 for(const [label,viewport] of [['desktop',{width:1280,height:900}],['iphone',{width:390,height:844}]]){
  ...
  check(`[${label}] dossier shows uncertainty-aware staff read`,/STAFF READ/.test(dossier)&&/confidence/i.test(dossier)&&await page.locator('.rw-confidence').count()===1);
  check(`[${label}] dossier shows recruiting battle context`,/RECRUITING BATTLE/.test(dossier)&&/Your position/.test(dossier)&&/Gap to leader/.test(dossier)&&/Momentum/.test(dossier));
  check(`[${label}] dossier labels evaluation stage`,/Evaluation stage/.test(dossier));
  ...
  await page.click('#rwSwitch [data-rwm="table"]');await page.waitForFunction(()=>!document.querySelector('#recruiting > .table-wrap')?.classList.contains('recruit-table-hidden'));check(`[${label}] Table mode restores dense legacy board`,await page.locator('#recruiting > .table-wrap').isVisible()&&!(await page.locator('#recruitWorkspace').isVisible()));
