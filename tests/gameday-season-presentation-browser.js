...
(async()=>{
 ...
 for(const [label,viewport] of [['desktop',{width:1280,height:900}],['iphone',{width:390,height:844}]]){
  ...
  await goTab(page,'gamelab');await page.waitForSelector('#v2MatchupIntelligence',{state:'visible',timeout:15000});
  const mount=await page.evaluate(()=>{const next=document.querySelector('#nextGameCard'),hero=document.querySelector('#gamedayEventHero'),data=next?.querySelector('.gameday-data');return{hasNext:!!next,hasData:!!data,dataWeek:data?.dataset?.week||null,hasHero:!!hero,heroHidden:hero?.hidden??null,heroText:hero?.textContent?.trim().slice(0,180)||'',gamelabActive:document.querySelector('#gamelab')?.classList.contains('active')||false}});console.log(`MOUNT [${label}] ${JSON.stringify(mount)} errors=${JSON.stringify(errors)}`);
  await page.waitForSelector('#gamedayEventHero:not([hidden])',{timeout:5000});const event=(await page.locator('#gamedayEventHero').innerText()).toLowerCase();
