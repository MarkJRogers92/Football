...
(async()=>{
 ...
 for(const [label,viewport] of [['desktop',{width:1280,height:900}],['iphone',{width:390,height:844}]]){
  ...
  await goTab(page,'gamelab');await page.waitForSelector('#v2MatchupIntelligence',{state:'visible',timeout:15000});await page.waitForFunction(()=>!!globalThis.DynastyLabGameDayPresentation?.snapshot?.(),{timeout:10000});await page.waitForSelector('#gamedayEventHero:not([hidden])',{timeout:10000});
  const snapshot=await page.evaluate(()=>globalThis.DynastyLabGameDayPresentation?.snapshot?.());const event=(await page.locator('#gamedayEventHero').innerText()).toLowerCase();
  check('read-only Game Day snapshot resolves supported matchup facts',!!snapshot?.user?.id&&!!snapshot?.opponent?.id&&!!snapshot?.venue&&!!snapshot?.stakes,JSON.stringify({week:snapshot?.week,user:snapshot?.user?.name,opponent:snapshot?.opponent?.name,venue:snapshot?.venue}));
