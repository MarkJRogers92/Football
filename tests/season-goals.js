...
test('season goals are contextual, persistent, and include a primary wins target',async()=>{
 const e=await setup(),t=e.T('Chicago Metropolitan'),g=e.ensureSeasonGoals(t);
 assert.ok(g.length>=4&&g.length<=5);
...
test('completed secondary goals move the administration review',async()=>{
 ...
 const blue=goals.find(x=>x.type==='bluechips');
 for(let i=0;i<blue.target;i++)e.universe.recruits[i].stars=4,e.universe.recruits[i].committed=t.name;
