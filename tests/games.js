...
test('permanent boxes match actual player deltas, snapshot identity and retain detailed drives',async()=>{
 ...
 assert.ok(g.drives.length>=18&&g.drives.length<=30);const sideDrives={home:g.drives.filter(d=>d.side==='home').length,away:g.drives.filter(d=>d.side==='away').length};assert.ok(Math.abs(sideDrives.home-sideDrives.away)<=1);assert.equal(g.drives.reduce((n,d)=>n+d.points,0)+g.scoreAdjustment.home+g.scoreAdjustment.away,g.score.home+g.score.away);
