...
test('permanent boxes match actual player deltas, snapshot identity and retain detailed drives',async()=>{
 ...
 for(const d of g.drives){const credited=g.playerStats[d.side],defenders=g.playerStats[d.side==='home'?'away':'home'];for(const line of d.playByPlay||[]){let m=line.match(/^.+? to (.+?) for -?\d+/)||line.match(/^.+? incomplete for (.+?)(?:;|\.)/)||line.match(/^.+? intercepts .+? targeting (.+?) near/);if(m)assert.ok(credited.some(x=>x.name===m[1]&&(x.stats.targets||0)>0),`${m[1]} owns a target credited by the play log`);m=line.match(/^(.+?) (?:keeps|runs) for -?\d+/);if(m)assert.ok(credited.some(x=>x.name===m[1]&&(x.stats.rushAtt||0)>0),`${m[1]} owns a carry credited by the play log`);m=line.match(/^(.+?) (?:field goal good|misses the field goal)/);if(m)assert.ok(credited.some(x=>x.name===m[1]&&(x.stats.fgAtt||0)>0),`${m[1]} owns the field-goal attempt named by the play log`);m=line.match(/^(.+?) punts\.$/);if(m)assert.ok(credited.some(x=>x.name===m[1]&&(x.stats.punts||0)>0),`${m[1]} owns the punt named by the play log`);m=line.match(/^(.+?) sacks .+? for/);if(m)assert.ok(defenders.some(x=>x.name===m[1]&&(x.stats.sacks||0)>0),`${m[1]} owns the sack named by the play log`);m=line.match(/^(.+?) intercepts /);if(m)assert.ok(defenders.some(x=>x.name===m[1]&&(x.stats.intDef||0)>0),`${m[1]} owns the interception named by the play log`);m=line.match(/; tackle by (.+?)\.$/);if(m)assert.ok(defenders.some(x=>x.name===m[1]&&(x.stats.tackles||0)>0),`${m[1]} owns the tackle named by the play log`);m=line.match(/; broken up by (.+?)\.$/);if(m)assert.ok(defenders.some(x=>x.name===m[1]&&(x.stats.passBreakups||0)>0),`${m[1]} owns the breakup named by the play log`)}}
