...
test('permanent boxes match actual player deltas, snapshot identity and retain detailed drives',async()=>{
 ...
 assert.ok(g.drives.length>=18&&g.drives.length<=30);const sideDrives={home:g.drives.filter(d=>d.side==='home').length,away:g.drives.filter(d=>d.side==='away').length};assert.ok(Math.abs(sideDrives.home-sideDrives.away)<=1);assert.ok(g.drives.every(d=>Array.isArray(d.playByPlay)&&d.playByPlay.length));assert.ok(g.drives.flatMap(d=>d.playByPlay).length>g.drives.length);assert.equal(g.drives.reduce((n,d)=>n+d.points,0)+g.scoreAdjustment.home+g.scoreAdjustment.away,g.score.home+g.score.away);
 const withoutPlay=JSON.parse(JSON.stringify(g));withoutPlay.drives.forEach(d=>delete d.playByPlay);const playBytes=Buffer.byteLength(JSON.stringify(g))-Buffer.byteLength(JSON.stringify(withoutPlay));assert.ok(playBytes>0&&playBytes<30000,`play-by-play should remain bounded, got ${playBytes} bytes`);console.log('Detailed play-by-play extra bytes/game:',playBytes);
