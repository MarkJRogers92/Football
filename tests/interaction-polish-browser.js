...
(async()=>{
 ...
 for(const [label,viewport] of [['desktop',{width:1280,height:900}],['iphone',{width:390,height:844}]]){
  ...
  const cards=page.locator('.recruit-workspace-card');if(await cards.count()>1){const before=(await page.locator('.recruit-workspace-dossier').innerText()).trim();await cards.nth(1).click();await page.waitForFunction(()=>document.querySelector('.recruit-workspace-dossier')?.classList.contains('dl-motion-detail'),{timeout:3000});const after=(await page.locator('.recruit-workspace-dossier').innerText()).trim();check('recruit dossier change receives restrained detail reveal',before!==after&&await page.locator('.recruit-workspace-dossier').evaluate(el=>el.classList.contains('dl-motion-detail')))}else check('recruit dossier change receives restrained detail reveal',false,'not enough prospect cards');
