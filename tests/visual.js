...
(async()=>{
 ...
 for(const [label,viewport] of [['desktop',{width:1280,height:900}],['iphone',{width:390,height:844}]]){
  ...
  await page.evaluate(()=>window.__DL_TEST__.goToTab('gamelab'));await page.waitForSelector('#nextGameCard .matchup-shell');
  check(`[${label}] Game Lab matchup card renders`,await page.locator('#nextGameCard .sports-mark').count()===2);
  await page.evaluate(()=>window.__DL_TEST__.goToTab('roster'));await page.click('#rosterBody .player-button');await page.waitForSelector('#playerDialog[open] .player-hero-rail');
  ...
  await page.evaluate(()=>window.__DL_TEST__.goToTab('dashboard'));await page.click('#simWeek');await page.waitForFunction(()=>/Week 1/.test(document.querySelector('#weekLine')?.textContent),{timeout:60000});await page.waitForSelector('#broadcastFeature .broadcast-matchup');
