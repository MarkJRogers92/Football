...
const playerFrom=(snap,extra={})=>({id:'P1',name:'Test Recruit',pos:snap.pos||'WR',perceived:78,perceivedUpside:86,scoutConfidence:74,trueNow:99,upside:99,growthProfile:'elite',recruitingMemory:{season:2027,schoolId:1,stars:snap.stars,scoutingReceipt:snap},stats:{games:8,starts:5},seasonHistory:[],awards:[],...extra});
...
test('team recap separates settled outcomes, watch cases and costly misreads',()=>{const {api,universe}=fixture(),low=api.signingSnapshot(recruit(),team(),verdict(),{}),high=api.signingSnapshot(recruit({id:'R2',stars:5,scout:84,scoutUp:91}),team(),verdict({score:88,label:'Priority take'}),{full:true,receipts:[{cost:3}]});universe.year=2029;const diamond=playerFrom(low,{id:'P1',name:'Diamond Guy',perceived:84,stats:{games:14,starts:11}}),bust=playerFrom(high,{id:'P2',name:'Bust Guy',perceived:66,stats:{games:14,starts:2}}),pending=playerFrom(low,{id:'P3',name:'Freshman',stats:{games:1,starts:0},scoutConfidence:50});const summary=api.summarizePlayers([diamond,bust,pending],2029,1);assert.equal(summary.tracked,3);assert.equal(summary.final,2);assert.equal(summary.pending,1);assert.equal(summary.counts.DIAMOND,1);assert.equal(summary.counts.BUST,1);assert.equal(summary.costly,1);assert.equal(summary.notable[0].result.final,true)});
