...
| 7 | **Recruiting board is a 14-column spreadsheet.** 44px portraits, every cell the same weight, stars/rank/interest not scannable, commitment state buried in the Interest column. Three stacked cards (class summary, signing class, battles) precede the table. | Med | High | Medium | Medium | DONE |
...
| 10 | **Shell and mobile chrome are heavy.** 14 flat tabs overflow at 1280 and 390 with only an edge fade; on 390px the topbar wraps five save/load buttons into two rows (~110px) above a status strip, so the first screen shows almost no game. | Med | High | Medium | Medium | DONE |
...
- **7 (Recruiting) DONE in v0.9.22** — the national board is the default working
  surface; My Battles and Signing Class are local views instead of stacked blocks.
  Each prospect has a position-aware projected range and confidence meter, while
  rank, stars, interest, commitment and NIL actions retain the established row
  hierarchy. Portrait size remains deliberately compact for the 220-row board.
- **10 (Shell) DONE in v0.9.22** — five primary areas drive a contextual second
  row containing the existing 14 destinations. Direct panel navigation still
  selects the correct parent area, and the compact mobile header remains intact.
  A forced first-run path was not added because the grouped shell resolves the
  navigation issue without changing onboarding or simulation flow.
