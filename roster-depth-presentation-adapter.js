...
if(typeof module==='object'&&module.exports){module.exports={makeRosterDepthPresentationModel};}
 ...
 function presentationPlayer(t,p){return{id:p.id,name:p.name,pos:p.pos,jerseyNumber:p.jerseyNumber??0,eligibility:eligibilityLabel(p),height:heightStr(p.height),weight:p.weight,style:p.style,currentRead:Number.isFinite(Number(p.perceived))?Math.round(Number(p.perceived)):null,currentLabel:scoutRangeText(p,t,false),upsideLabel:scoutRangeText(p,t,true),health:p.health,healthLabel:healthText(p),injuryWeeks:p.injuryWeeks||0,redshirtActive:!!p.redshirtActive,academicLabel:academicStatusText(p),morale:p.morale,roles:presentationRolesForPlayer(t,p)};}
