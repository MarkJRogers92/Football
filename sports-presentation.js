...
function dashboard(){
 const d=document.querySelector('#dashboard'),hero=d?.querySelector('.hero-grid'),hub=document.querySelector('#weeklyHub');if(!d||!hero||!hub)return;
...
let queued=false;function run(){queued=false;dashboard();playerHero();matchup();score()}function queue(){if(queued)return;queued=true;if(window.requestAnimationFrame)window.requestAnimationFrame(run);else setTimeout(run,0)}
document.addEventListener('click',e=>{const tab=e.target?.closest?.('[data-sports-tab]')?.dataset?.sportsTab;if(tab){e.preventDefault();go(tab)}setTimeout(queue,0)});document.querySelector('#userTeam')?.addEventListener('change',()=>setTimeout(queue,0));
if(window.MutationObserver){for(const sel of ['#weeklyHub','#playerDialog','#nextGameCard','#detailedBox']){const el=document.querySelector(sel);if(el)new MutationObserver(queue).observe(el,{childList:true,subtree:true,characterData:true,attributes:sel==='#playerDialog'})}}
queue();
})();
