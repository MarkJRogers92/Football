...
let queued=false;function run(){queued=false;dashboard();top15();playerHero();matchup();score()}function queue(){if(queued)return;queued=true;if(window.requestAnimationFrame)window.requestAnimationFrame(run);else setTimeout(run,0)}
...
if(window.MutationObserver){for(const sel of ['#weeklyHub','#top15','#playerDialog','#nextGameCard','#detailedBox']){const el=document.querySelector(sel);if(el)new MutationObserver(queue).observe(el,{childList:true,subtree:true,characterData:true,attributes:sel==='#playerDialog'})}}
queue();
})();
