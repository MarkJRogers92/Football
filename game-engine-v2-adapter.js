...
function eventSummary(state){
  ...
  for(const e of state.events||[]){
    ...
    else if(e.type==='interception'||e.type==='fumble')out[t].turnovers++;
...
function profileOutcome(state,homeInput,awayInput){
  const rng=DynastyRng.create(state.rng),offense=state.possession==='home'?homeInput:awayInput,defense=state.possession==='home'?awayInput:homeInput;
  const edge=(offense.offense-defense.defense)/18,roll=rng.next(),pass=rng.next()<.53;
  const intRate=clamp(.018+(defense.defense-offense.offense)*.00022,.008,.04);
  const fumbleRate=clamp(.014+(defense.defense-offense.offense)*.00010,.007,.028);
  let outcome;
  if(roll<intRate)outcome={turnover:'interception',yards:clamp(Math.round(8+rng.gauss()*9),-5,35),clock:rng.int(4,12)};
  else if(roll<intRate+fumbleRate)outcome={turnover:'fumble',yards:clamp(Math.round(3+rng.gauss()*6),-8,18),clock:rng.int(5,14)};
  else{
    const mean=pass?5.6:4.3,spread=pass?8.2:4.8,edgeWeight=pass?1:.7;
    outcome={yards:clamp(Math.round(mean+edge*edgeWeight+rng.gauss()*spread),-14,55),clock:pass?rng.int(5,34):rng.int(22,40),kind:pass?'pass':'rush'};
  }
...
function simulateShadow(options={}){
  const {state,homeInput,awayInput}=createShadowGame(options);GameEngineV2.startGame(state);
  let steps=0;while(state.status!=='final'){
    ...
    else GameEngineV2.applyScrimmage(state,profileOutcome(state,homeInput,awayInput));
  }
  GameEngineV2.validateGame(state);return{state,summary:eventSummary(state),inputs:{home:homeInput,away:awayInput}};
}
