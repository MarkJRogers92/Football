// Development-only Game Lab bridge for the clock-aware v2 engine, real-player attribution and transactional dry run.
...
function v2LabIntegrity(g,home,away){return JSON.stringify({
  ...
  home:{w:home.w,l:home.l,cw:home.cw,cl:home.cl,pf:home.pf,pa:home.pa,sos:home.sos,playerStats:v2RosterStatChecksum(home)},
  away:{w:away.w,l:away.l,cw:away.cw,cl:away.cl,pf:away.pf,pa:away.pa,sos:away.sos,playerStats:v2RosterStatChecksum(away)}
})}
...
function v2StarterIds(team){
  const roles=['QB1','RB1','X','Z','SLOT','TE1','LT','LG','C1','RG','RT','RUSH','SETEDGE','NT','3TECH','MIKE','WILL','NICKEL','BCB','FCB','FS','BOXS'];
  return [...new Set(roles.map(id=>roleStarter(team,id)).filter(Boolean).map(p=>p.id))];
}
...
function runV2GameLabShadow(){
  ...
  if(!api||!actors||!tx){setStatus('Game Engine 2 development modules are unavailable in this build.');return}
  ...
  const attribution=actors.attributeGame(preview.state,{home:v2AttributionContext(hc),away:v2AttributionContext(ac)});
  actors.attachEventSummary(attribution,preview.summary);preview.attribution=attribution;
  const candidate=tx.buildCandidate({
    state:preview.state,attribution,homeTeam:hc,awayTeam:ac,
    starterIds:{home:v2StarterIds(hc),away:v2StarterIds(ac)},
    meta:{season:universe.year,week:g.week??universe.week+1,phase:universe.phase,label:'Regular season',venue:`${home.name} · ${home.city||''}`},
    conference:g.conf??home.conference===away.conference,
    homeOpponentOverall:awayProfile.overall,awayOpponentOverall:homeProfile.overall
  });
  preview.transactionProof=tx.dryRunTransaction({candidate,homeTeam:hc,awayTeam:ac,homeGameplan:gameplanSnapshot(hc,ac.name),awayGameplan:gameplanSnapshot(ac,hc.name)});
...
function v2TransactionHTML(preview){
  const proof=preview?.transactionProof;if(!proof)return'';
  return`<div class="v2-transaction-proof"><div><strong>Archive transaction dry run: PASS</strong><div class="small muted">The v2 result was applied to cloned team/player state and validated against the existing game archive contract.</div></div><div class="v2-proof-records"><span>${gameEscape(proof.applied.awayRecord)}</span><span>·</span><span>${gameEscape(proof.applied.homeRecord)}</span></div></div>`
}
function renderV2GameLabPanel(){
  ...
  const disabled=!g||!api||!globalThis.DynastyGameEngineV2Attribution||!globalThis.DynastyGameEngineV2Transaction;
  host.innerHTML=`<div class="section-head v2-lab-head"><div><div class="eyebrow">V0.10.1 DEVELOPMENT PREVIEW</div><h3>Game Engine 2 Shadow</h3><div class="muted">Clock-aware football, real-player attribution and archive transaction validation — nothing is recorded.</div></div><button type="button" data-v2-shadow-run ${disabled?'disabled':''}>${v2GameLabPreview?'Replay Same Seed':'Run Shadow Preview'}</button></div>${api?api.summaryHTML(v2GameLabPreview):'<div class="muted">V2 Lab module unavailable.</div>'}${v2TransactionHTML(v2GameLabPreview)}${v2AttributionHTML(v2GameLabPreview)}<div class="v2-shadow-safety"><strong>Shadow only.</strong> Standings, roster statistics, the scheduled game and browser save are untouched.</div>${v2GameLabPreview?`<div class="v2-event-head"><strong>Clock-aware event stream</strong><span class="small muted">Newest first · deterministic seed</span></div><div class="v2-event-feed">${api.eventsHTML(v2GameLabPreview,160)}</div>`:''}`;
...
globalThis.DynastyGameEngineV2LabBridge={runShadow:runV2GameLabShadow,getPreview:()=>v2GameLabPreview,render:renderV2GameLabPanel,contextForTeam:v2AttributionContext,starterIds:v2StarterIds};
