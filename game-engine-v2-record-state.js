// v0.10.1 development-only recorded Game Engine 2 state/rollback helpers.
...
function v2RecordDriveArchive(preview){
  const api=globalThis.DynastyGameEngineV2Lab,events=preview?.state?.events||[],names=preview?.names||{},counts={home:0,away:0},drives=[];let drive=null;
  ...
  const start=side=>{if(valid(side))drive={side,label:`${side==='home'?'H':'A'}${++counts[side]}`,points:0,result:'',plays:0,playByPlay:[]}};
  for(const e of events){
    if(e.type==='kickoff'&&valid(e.receiver)){finish();start(e.receiver);continue}
    if(e.type==='overtime_possession'&&valid(e.possession)){finish();start(e.possession);continue}
    if(e.type==='possession_change'&&valid(e.to)){if(drive&&drive.side!==e.to)finish();if(!drive)start(e.to);continue}
    const side=valid(e.team)?e.team:null;if(side&&(!drive||drive.side!==side)){finish();start(side)}if(!drive)continue;
    if(['scrimmage','interception','fumble'].includes(e.type)){drive.plays++;drive.playByPlay.push(api.eventText(e,names));if(e.type==='interception')drive.result='INT';if(e.type==='fumble')drive.result='FUMBLE'}
    else if(e.type==='touchdown'){drive.points+=6;drive.result='TD';drive.playByPlay.push(api.eventText(e,names))}
    else if(e.type==='extra_point'){if(e.made!==false)drive.points++;drive.playByPlay.push(api.eventText(e,names))}
    else if(e.type==='two_point'){if(e.made)drive.points+=2;drive.playByPlay.push(api.eventText(e,names))}
    else if(e.type==='field_goal'){if(e.made){drive.points+=3;drive.result='FG'}else drive.result='MISS';drive.playByPlay.push(api.eventText(e,names))}
    else if(e.type==='punt'){drive.result='PUNT';drive.playByPlay.push(api.eventText(e,names))}
    else if(e.type==='safety'){drive.points+=2;drive.result='SAFETY';drive.playByPlay.push(api.eventText(e,names))}
  }
