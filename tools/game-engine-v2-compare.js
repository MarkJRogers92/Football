...
async function main(){
 ...
 for(let i=0;i<matchups.length;i++){
  ...
  const homeProfile=e.gameProfiles(ph,pa.name),awayProfile=e.gameProfiles(pa,ph.name);
  ...
  const box=old.box||{};
  oldRows.push({
   points:(old.hp||0)+(old.ap||0),
   homePoints:old.hp||0,awayPoints:old.ap||0,
   ...
   homeWin:(old.hp||0)>(old.ap||0)?1:0,
   profileGap:round(homeProfile.overall-awayProfile.overall)
  });
  if(u.gameArchive)u.gameArchive.length=archiveLen;if(u.events)u.events.length=eventLen;u.gameCounter=counter;

  const shadow=adapter.simulateShadow({gameId:`cal-${i}-${home.id}-${away.id}`,seed:`v0100-cal-${i}-${home.id}-${away.id}`,home,away,homeProfile,awayProfile});
  v2Rows.push({
   points:shadow.summary.totalPoints,
   homePoints:shadow.summary.home.points,awayPoints:shadow.summary.away.points,
   plays:shadow.summary.totalPlays,
   yards:shadow.summary.home.yards+shadow.summary.away.yards,
   turnovers:shadow.summary.home.turnovers+shadow.summary.away.turnovers,
   homeWin:shadow.state.score.home>shadow.state.score.away?1:0,
   profileGap:round(homeProfile.overall-awayProfile.overall)
  });
 }
 const summarize=rows=>({games:rows.length,meanPoints:round(mean(rows,'points')),meanPlays:round(mean(rows,'plays')),meanYards:round(mean(rows,'yards')),meanTurnovers:round(mean(rows,'turnovers')),homeWinRate:round(mean(rows,'homeWin'))});
