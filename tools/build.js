...
if (packageVersion !== version) throw new Error(`version drift: VERSION.txt says ${version}, package.json version says ${packageVersion}`);
...
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8">` +
  ...
  read('styles.css')+'\n'+read('visual-identity.css')+'\n'+read('team-branding.css')+'\n'+read('conference-branding.css')+'\n'+read('sports-presentation.css')+'\n'+read('sports-layout.css')+'\n'+read('recruit-presentation.css')+'\n'+read('recruiting-workspace.css')+'\n'+read('scouting-actions.css')+'\n'+read('staff-scouting.css')+'\n'+read('scouting-trail.css')+'\n'+read('recruiting-filters.css')+'\n'+read('recruit-compare.css')+'\n'+read('recruiting-shortlist.css')+'\n'+read('scouting-receipts.css')+'\n'+read('recruiting-history.css')+'\n'+read('development-tendencies.css')+'\n'+read('recruiting-battles.css')+'\n'+read('development-plans.css')+'\n'+read('game-engine-v2-lab.css')+'\n'+read('game-engine-v2-presentation.css')+'\n'+read('game-engine-v2-gameday.css')+'\n'+read('game-engine-v2-matchup.css')+'\n'+read('weekly-coaching-loop.css')+'\n'+read('weekly-player-stories.css')+'\n'+read('polish.css')+'\n'+read('client-shell.css')+
  `</style></head><body>\n`+body+`<script>`+read('portraits/renderer-v1.js')+'\n'+read('storage.js')+'\n'+read('rng.js')+'\n'+read('game-engine-v2.js')+'\n'+read('game-engine-v2-decisions.js')+'\n'+read('game-engine-v2-adapter.js')+'\n'+read('game-engine-v2-gameday.js')+'\n'+read('game-engine-v2-attribution.js')+'\n'+read('game-engine-v2-transaction.js')+'\n'+read('game-engine-v2-lab.js')+'\n'+read('game-engine-v2-gameplan-feedback.js')+'\n'+read('weekly-coaching-engine.js')+'\n'+read('weekly-player-stories-engine.js')+'\n'+read('escape.js')+'\n'+appSource+'\n'+read('program-branding.js')+'\n'+read('conference-branding.js')+'\n'+read('visual-identity.js')+'\n'+read('sports-presentation.js')+'\n'+read('recruit-presentation.js')+'\n'+read('recruiting-workspace.js')+'\n'+read('next-action.js')+'\n'+read('team-logo-coverage.js')+'\n'+read('program-color-coverage.js')+'\n'+read('client-shell.js')+`</script></body></html>`;
const out = process.argv[2] || 'index.html';fs.writeFileSync(path.join(root, out), html);console.log(`built ${out} (${(html.length / 1024).toFixed(0)} KB) with Game Engine 2 foundation, interactive coaching decisions, matchup intelligence, v0.10.2 opponent scouting, weekly preparation, Staff Room, selective personnel decisions and factual Player Story Cards, archived game-plan feedback, permanent Game Day recording and season soak coverage, calibrated matchup simulation, real-player attribution, transactional rollback, release Game Day presentation, scouting, recruiting intelligence, development systems and the premium client shell`);