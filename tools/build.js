...
const out = process.argv[2] || 'index.html';fs.writeFileSync(path.join(root, out), html);console.log(`built ${out} (${(html.length / 1024).toFixed(0)} KB) with scouting, staff evaluation identity, evaluation trail, recruiting filters, recruit compare, staff shortlist, delayed scouting receipts, recruiting history, recruiting battles, observed development tendencies and featured development plans`);
