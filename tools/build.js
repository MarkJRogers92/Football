...
// / visual-identity.css / team-branding.css / visual-identity.js / body.html / storage.js /
...
const html = `<!doctype html><html lang="en"><head><meta charset="utf-8">` +
  ...
  `<script>` + read('portraits/renderer-v1.js') + '\n' + read('storage.js') + '\n' + read('app.js') + '\n' + read('visual-identity.js') + `</script></body></html>`;

const out = process.argv[2] || 'index.html';
fs.writeFileSync(path.join(root, out), html);
console.log(`built ${out} (${(html.length / 1024).toFixed(0)} KB) from renderer-v1.js + app.js + storage.js + styles.css + visual-identity.css + team-branding.css + body.html + visual-identity.js`);
