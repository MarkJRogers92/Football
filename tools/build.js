...
for(const file of ['scouting-actions.js','recruit-compare.js','recruiting-shortlist.js'])appSource=injectEngineExtension(appSource,file);
...
const out = process.argv[2] || 'index.html';fs.writeFileSync(path.join(root, out), html);console.log(`built ${out} (${(html.length / 1024).toFixed(0)} KB) with scouting, recruit compare and staff shortlist extensions`);
