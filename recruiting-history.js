function makeRecruitingHistorySystem(env){
 'use strict';
 function playerPool(team,archive=[]){
  const active=team?.roster||[],schoolId=team?.id,archived=(archive||[]).filter(p=>p?.recruitingMemory?.scoutingReceipt?.schoolId===schoolId),seen=new Set();
  return [...active,...archived].filter(p=>{if(!p?.id||seen.has(p.id))return false;seen.add(p.id);return true});
 }
...
if(typeof module==='object'&&module.exports){
 ...
 globalThis.DynastyLabRecruitingHistory={history:t=>recruitingHistorySystem.history(recruitingHistoryPlayers(t),universe.year,t.id)};
}
