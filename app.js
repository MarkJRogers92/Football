...
function recruitScoutingConfidence(r,t){const stored=Object.entries(r.scoutingDomains||{}).map(([key,x])=>Math.max(x.confidence||0,scoutingConfidence(r,t,[key,'',null],true)));return clamp(Math.round(stored.length?avg(stored):scoutingConfidence(r,t,scoutingDefs(r)[0],true)),20,96)}
function recruitScoutingReadHTML(r,t){const confidence=recruitScoutingConfidence(r,t),width=clamp(Math.round((100-confidence)/6),2,14),low=clamp(r.scout-width,25,99),high=clamp(r.scout+width,25,99),label=scoutingConfidenceLabel(confidence);return `<div class="recruit-scout-read"><strong>${grade(r.scout)} · ${low}–${high}</strong><span class="recruit-confidence" aria-label="${label} scouting confidence, ${confidence} percent"><span class="confidence-meter"><i style="width:${confidence}%"></i></span><span>${label}</span></span></div>`}
...
function gameAvailable(p){return !!p&&(p.injuryWeeks||0)===0&&!p.redshirtActive&&weeklyPlayerPlan(p)?.role!=='sit'}
...
function reviewControlledProgram(t){
 if(!t)return null;
 universe.tenure??={startYear:universe.year,school:t.name,seasons:[],ended:null};
...
function signNilDeal(t,x,isRecruit){
 ...
 ensureNilState(t);
 if(x.nilDeal?.year===universe.year)return {ok:false,reason:'A deal is already in place this year.'};
...
function nilRecruitHTML(r,t){
 const current=r.nilDeal?.year===universe.year?r.nilDeal:null,cost=nilDealCost(r,true),afford=cost<=nilRemaining(t);
 if(current?.schoolId===t.id)return `<button class="rs-button rs-active" data-nil-recruit="${r.id}" title="Release this offer">NIL held · ${current.amount}</button>`;
 if(current)return '<span class="small muted">NIL committed</span>';
 return `<button class="rs-button" data-nil-recruit="${r.id}" ${afford?'':'disabled'} title="${afford?`Costs ${cost}`:'Not enough NIL left'}">NIL · ${cost}</button>`;
}
...
const NAV_GROUPS={home:['dashboard'],team:['roster','depth','staff','development'],competition:['gamelab','season','newsletter','stats'],recruiting:['recruiting'],program:['program','offseason','records','history']};
function navGroupFor(id){return Object.keys(NAV_GROUPS).find(group=>NAV_GROUPS[group].includes(id))||'home'}
function syncNavigation(id){const group=navGroupFor(id),shell=$('.tabs-shell'),tabs=$('.tabs');if(shell)shell.dataset.activeGroup=group;if(tabs)tabs.setAttribute('aria-label',`${group[0].toUpperCase()+group.slice(1)} views`);$$('[data-nav-group]').forEach(x=>{const on=x.dataset.navGroup===group;x.classList.toggle('active',on);x.setAttribute('aria-current',on?'page':'false')});$$('.tabs button[data-tab]').forEach(x=>{const show=x.dataset.navParent===group;x.classList.toggle('nav-visible',show);x.hidden=!show;x.classList.toggle('active',x.dataset.tab===id)})}
function renderActiveTab(){(TAB_RENDERERS[activeTab]||(()=>{}))()}
function setActiveTab(id){activeTab=id;syncNavigation(id);renderActiveTab()}
function goToTab(id){if(!$('#'+id))return;$$('.tab').forEach(x=>x.classList.toggle('active',x.id===id));setActiveTab(id);$('#'+id).scrollIntoView?.({block:'start'})}
...
function hasPendingWeeklyDecisions(t=selected()){return !!t&&(universe.weeklyDecisions||[]).some(d=>d.season===universe.year&&d.week===universe.week&&d.teamId===t.id&&!d.resolved)}
...
function resolveWeeklyDecision(id,optionId){const d=(universe.weeklyDecisions||[]).find(x=>x.id===id),t=d&&universe.teams.find(x=>x.id===d.teamId),option=d?.options.find(x=>x.id===optionId);if(!d||d.resolved||!t||!option)return false;const p=d.playerId?t.roster.find(x=>x.id===d.playerId):null;
 if(d.type==='INJURED_STARTER'&&p)p.weeklyAvailability={season:d.season,week:d.week,role:option.id};
...
function renderProgram(){const u=selected();$('#editName').value=u.name;$('#editConference').value=u.conference;$('#editPrestige').value=u.prestige;$('#editResources').value=u.resources;$('#editNil').value=u.nil;$('#editDevelopment').value=u.development;$('#editFacilities').value=u.facilities;$('#editAcademics').value=u.academics;$('#programDNA').innerHTML=[['Prestige',u.prestige],['Resources',u.resources],['NIL',u.nil],['Development',u.development],['Facilities',u.facilities],['Academics',u.academics],['Fan Support',u.fan_support],['Admin Patience',u.admin_patience],['Program Ceiling',u.program_ceiling],['Rivalry',u.rivalry?(rivalOf(u)?.name||'—'):'—'],['Trophy',u.rivalry?.trophy||'—'],['All-time series',rivalrySeriesText(u)||'—'],['Season expectation',seasonExpectation(u)+' wins'],['Admin confidence',(u.adminConfidence??'—')+' · '+adminConfidenceLabel(u.adminConfidence??50)],['Mandate',u.mandate?u.mandate.text:'None'],['NIL budget',nilSummaryText(u)]].map(x=>`<div class="lineitem"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join('')}
...
function bind(){
 ...
 $$('.tabs button[data-tab]').forEach(b=>b.onclick=()=>goToTab(b.dataset.tab));
 $$('[data-nav-group]').forEach(b=>b.onclick=()=>goToTab(b.dataset.navTarget));
 $$('[data-recruit-view]').forEach(b=>b.onclick=()=>setRecruitView(b.dataset.recruitView));
 syncNavigation(activeTab);setRecruitView(recruitView);
...
if(typeof window!=='undefined')window.__DL_TEST__={selected,createOpening,renderStaff,setTeamScheme,schemeTransition,positionChangeWillingness,goToTab,setRecruitView};
