...
function detailedGame(home,away,neutral=false,confOverride=null,context={}){
 ...
 let hp=0,ap=0;const driveCount=detailedDriveCount(hScheme,aScheme),openingSide=Math.random()<.5?'home':'away',sideCounts={home:0,away:0};for(let i=0;i<driveCount;i++){const side=i%2?(openingSide==='home'?'away':'home'):openingSide,isHome=side==='home',label=`${isHome?'H':'A'}${++sideCounts[side]}`,d=drive(isHome?home:away,isHome?away:home,isHome?H:A,isHome?A:H,isHome?hScheme:aScheme,totals[side],label);if(isHome)hp+=d.points;else ap+=d.points;drives.push({side,label,...d})}if(!neutral)hp+=Math.random()<.55?3:0;if(hp===ap){log.push('Overtime required.');if(Math.random()<.5)hp+=3;else ap+=3}const conf=confOverride??home.conference===away.conference,win=recordGame(home,away,hp,ap,conf,H,A);totals.home.pts=hp;totals.away.pts=ap;applyGameStats(home,totals.home,totals.away);applyGameStats(away,totals.away,totals.home);postGameCondition(home);postGameCondition(away);return finishGame(before,home,away,{home:home.name,away:away.name,hp,ap,winner:win.name,log:log.slice(-160),box:totals,detailed:true,drives});
}
...
function simulateUserDetailed(){const g=findUserGame();if(!g)return;if(hasPendingCareerChoice()){setStatus('Choose your next job before advancing the week.');return}if(hasPendingWeeklyDecisions()){setStatus('Resolve the Coach’s Desk decisions before playing this week.');return}recoverWeek();const r=detailedGame(T(g.home),T(g.away),false,g.conf);g.gameId=r.gameId;g.played=true;g.score=[r.ap,r.hp];g.winner=r.winner;g.detailed=true;universe.lastDetailedGame=r;universe.latest=[r];ranked();render()}
