...
function inheritRecruitTraits(p){if(p.recruitTraitsInherited||!universe?.recruits?.length)return false;const r=universe.recruits.find(r=>r.name===p.name&&r.pos===p.pos&&r.portraitSeed===p.portraitSeed&&hasPlayerTraits(r));if(!r)return false;Object.assign(p,playerTraitFields(r));p.recruitTraitsInherited=true;return true}
...
function offseasonReview(){
 ...
 for(const t of universe.teams){
  ...
  recordSeasonInHistory(t,t.w,t.l,universe.confChamps.some(c=>c.name===t.name),universe.champion===t.name);
