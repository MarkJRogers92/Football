...
function applyGameStats(team,s,opp){const q=roleStarter(team,'QB1')||starter(team,'QB');let core=[q,roleStarter(team,'RB1'),roleStarter(team,'X'),roleStarter(team,'Z'),roleStarter(team,'SLOT'),roleStarter(team,'TE1'),roleStarter(team,'LT'),roleStarter(team,'LG'),roleStarter(team,'C1'),roleStarter(team,'RG'),roleStarter(team,'RT'),roleStarter(team,'RUSH'),roleStarter(team,'SETEDGE'),roleStarter(team,'NT'),roleStarter(team,'3TECH'),roleStarter(team,'MIKE'),roleStarter(team,'WILL'),roleStarter(team,'NICKEL'),roleStarter(team,'BCB'),roleStarter(team,'FCB'),roleStarter(team,'FS'),roleStarter(team,'BOXS')].filter(Boolean);const starters=new Set();for(const p of [...new Map(core.map(p=>[p.id,p])).values()]){addGameAppearance(p,true);starters.add(p.id)}
 ...
 let qbRun=qbRushWeight(q,team),rusherPart=(p,weight)=>({p,weight:p?.pos==='QB'?weight:playerUsageWeight(p,'carry',weight)});let rushers=mergeShares([rusherPart(roleStarter(team,'RB1'),.46),rusherPart(rotationAt(team,'RB',1),.19),rusherPart(roleStarter(team,'3DRB'),.11),rusherPart(roleStarter(team,'PWRB'),.10),rusherPart(rotationAt(team,'RB',2),.06),rusherPart(q,qbRun)],.62);let att=splitInteger(s.rushAtt||0,rushers),yds=splitInteger(Math.max(0,s.rushYds||0),rushers.map(x=>({p:x.p,weight:x.weight*(x.p.speed*.45+x.p.power*.35+x.p.iq*.20)}))),td=splitInteger(s.rushTD||0,rushers.map(x=>({p:x.p,weight:x.weight*(x.p.power*.6+x.p.composure*.4)})));for(const x of rushers){x.p.stats.rushAtt+=(att.get(x.p)||0);x.p.stats.rushYds+=(yds.get(x.p)||0);x.p.stats.rushTD+=(td.get(x.p)||0)}
 let receiverPart=(p,weight)=>({p,weight:playerUsageWeight(p,'target',weight)}),recs=mergeShares([receiverPart(roleStarter(team,'X'),.21),receiverPart(roleStarter(team,'Z'),.19),receiverPart(roleStarter(team,'SLOT'),.18),receiverPart(roleStarter(team,'TE1'),.14),receiverPart(roleStarter(team,'3DRB'),.08),receiverPart(roleStarter(team,'MOVETE'),.05),receiverPart(rotationAt(team,'WR',3),.07),receiverPart(rotationAt(team,'WR',4),.04),receiverPart(rotationAt(team,'TE',1),.04)],.36);let catches=splitInteger(s.passComp||0,recs.map(x=>({p:x.p,weight:x.weight*(x.p.technique*.55+x.p.iq*.30+x.p.composure*.15)}))),extraTargets=splitInteger(Math.max(0,(s.passAtt||0)-(s.passComp||0)),recs),ryds=splitInteger(Math.max(0,s.passYds||0),recs.map(x=>({p:x.p,weight:x.weight*(x.p.speed*.42+x.p.technique*.38+x.p.power*.2)}))),rtd=splitInteger(s.passTD||0,recs.map(x=>({p:x.p,weight:x.weight*(x.p.technique*.45+x.p.power*.3+x.p.composure*.25)})));for(const x of recs){let c=catches.get(x.p)||0,tg=c+(extraTargets.get(x.p)||0);x.p.stats.targets+=tg;x.p.stats.receptions+=c;x.p.stats.recYds+=(ryds.get(x.p)||0);x.p.stats.recTD+=(rtd.get(x.p)||0);x.p.stats.yac+=Math.round((ryds.get(x.p)||0)*rng(.25,.48));x.p.stats.drops+=Math.max(0,Math.round((tg-c)*clamp((90-x.p.technique)/250,0,.15)))}
