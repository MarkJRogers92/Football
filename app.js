...
function weightedPlayer(parts){if(!parts.length)return null;const total=parts.reduce((n,x)=>n+Math.max(.01,x.weight),0);let roll=Math.random()*total;for(const x of parts){roll-=Math.max(.01,x.weight);if(roll<=0)return x.p}return parts.at(-1).p}
...
function quickFieldGoals(team,attempts,O,D){const k=roleStarter(team,'K1')||starter(team,'K');let made=0;for(let i=0;i<attempts;i++){const distance=clamp(Math.round(39+(D.defense-O.offense)*.10+gauss()*6),22,59);if(Math.random()<fieldGoalChance(k,distance,i===attempts-1))made++}return made}
...
function detailedGame(home,away,neutral=false,confOverride=null,context={}){
 ...
 const isPass=Math.random()<pass;if(isPass){
  const target=weightedPlayer(receivingShares(off))||starter(off,'WR'),qs=exactOffenseLine(actors,qb),ts=exactOffenseLine(actors,target),pressure=clamp(.17+(D.front-O.ol)*.005+(DEF_SCHEMES[def.defScheme].pressure-70)*.0015+(dc.playCall-70)*.001-(oc.playCall-70)*.0008+(D.prepPressure||0),.08,.48),pressured=Math.random()<pressure,rushDef=pressured?(defensiveActor(def,'rush',['EDGE','DT','LB'])||starter(def,'EDGE')):null,rushStats=rushDef?exactDefenseLine(defenders,rushDef):null;
  if(rushStats)rushStats.pressures++;
  if(pressured&&Math.random()<.28){let loss=gi(3,10);pos=Math.max(1,pos-loss);out.sacksTaken++;qs.sacksTaken++;rushStats.sacks++;rushStats.tackles++;rushStats.tfl++;down++;toGo+=loss;last=null;log.push(`${label}: ${rushDef.name} sacks ${qb.name} for ${loss}.`)}
  else{out.passAtt++;qs.passAtt++;ts.targets++;let intP=clamp(.027+(D.coverage-O.qb)*.0014+(75-qb.iq)*.0007+(pressured?.009:0),.008,.09);if(Math.random()<intP){const thief=defensiveActor(def,'takeaway',['CB','S','LB'])||starter(def,'CB'),ds=exactDefenseLine(defenders,thief);out.int++;out.turnovers++;qs.int++;ds.intDef++;log.push(`${label}: ${thief.name} intercepts ${qb.name} targeting ${target.name} near the ${Math.min(99,pos+gi(5,22))}.`);return {points:0,result:'INT',plays}}let comp=clamp(.61+(O.qb-D.coverage)*.004+(qb.technique-70)*.0015+(target.technique-70)*.0007-(pressured?.07:0),.38,.81);if(Math.random()<comp){let y=Math.round(clamp(7+(O.skill-D.coverage)/8+gauss()*6,0,38));if(Math.random()<.055+(qb.speed+target.speed-140)*.0005)y+=gi(12,38);pos+=y;out.passComp++;out.passYds+=y;out.recYds+=y;qs.passComp++;qs.passYds+=y;ts.receptions++;ts.recYds+=y;ts.yac+=Math.round(y*rng(.25,.48));last={type:'pass',qs,ts};const tackler=pos<100?(defensiveActor(def,'tackle')||starter(def,'LB')):null;if(tackler)exactDefenseLine(defenders,tackler).tackles++;if(y>=toGo){out.firstDowns++;down=1;toGo=Math.min(10,100-pos)}else{down++;toGo-=y}log.push(`${label}: ${qb.name} to ${target.name} for ${y}${tackler?`; tackle by ${tackler.name}`:''}.`)}else{const dropped=Math.random()<clamp((90-target.technique)/250,0,.15);if(dropped)ts.drops++;const cover=!dropped&&Math.random()<.35?(defensiveActor(def,'coverage',['CB','S','LB'])||starter(def,'CB')):null;if(cover)exactDefenseLine(defenders,cover).passBreakups++;last=null;down++;log.push(`${label}: ${qb.name} incomplete for ${target.name}${cover?`; broken up by ${cover.name}`:''}.`)}}
 }
 else{const rusher=weightedPlayer(rushingShares(off))||starter(off,'RB'),rs=exactOffenseLine(actors,rusher);out.rushAtt++;rs.rushAtt++;let runEdge=(O.ol*.38+O.skill*.25+rusher.power*.15+rusher.speed*.12+oc.playCall*.10)-(D.front*.60+DEF_SCHEMES[def.defScheme].run*.20+dc.playCall*.20),y=Math.round(clamp(3.7+runEdge/20+gauss()*3.2,-4,24));if(Math.random()<.045+Math.max(0,rusher.speed-82)*.001)y+=gi(10,32);pos+=y;out.rushYds+=y;rs.rushYds+=y;last={type:'run',rs};const tackler=pos<100?(defensiveActor(def,'tackle')||starter(def,'LB')):null;if(tackler){const ds=exactDefenseLine(defenders,tackler);ds.tackles++;if(y<0)ds.tfl++}if(y>=toGo){out.firstDowns++;down=1;toGo=Math.min(10,100-pos)}else{down++;toGo=Math.max(1,toGo-y)}log.push(`${label}: ${rusher.name} ${rusher===qb?'keeps':'runs'} for ${y}${tackler?`; tackle by ${tackler.name}`:''}.`)}
 ...
 if(down>=4){if(pos>=62){const dist=117-pos,k=roleStarter(off,'K1')||starter(off,'K'),pct=fieldGoalChance(k,dist,true);out.fgAtt++;if(Math.random()<pct){out.fgMade++;log.push(`${label}: ${k.name} field goal good from ${Math.round(dist)}.`);return {points:3,result:'FG',plays}}log.push(`${label}: ${k.name} misses the field goal from ${Math.round(dist)}.`);return {points:0,result:'MISS',plays}}const p=roleStarter(off,'P1')||starter(off,'P');out.punts++;log.push(`${label}: ${p.name} punts.`);return {points:0,result:'PUNT',plays}}
 }return {points:0,result:'END',plays}}
 let hp=0,ap=0;const driveCount=detailedDriveCount(hScheme,aScheme),openingSide=Math.random()<.5?'home':'away',sideCounts={home:0,away:0};for(let i=0;i<driveCount;i++){const side=i%2?(openingSide==='home'?'away':'home'):openingSide,isHome=side==='home',label=`${isHome?'H':'A'}${++sideCounts[side]}`,logStart=log.length,d=drive(isHome?home:away,isHome?away:home,isHome?H:A,isHome?A:H,isHome?hScheme:aScheme,totals[side],label,exact[side],exactDef[isHome?'away':'home']);d.playByPlay=log.slice(logStart).map(x=>x.startsWith(label+': ')?x.slice(label.length+2):x);if(isHome)hp+=d.points;else ap+=d.points;drives.push({side,label,...d})}hp+=homeFieldScoreBonus(home,neutral);if(hp===ap){log.push('Overtime required.');if(Math.random()<.5)hp+=3;else ap+=3}const conf=confOverride??home.conference===away.conference,win=recordGame(home,away,hp,ap,conf,H,A);totals.home.pts=hp;totals.away.pts=ap;applyGameStats(home,totals.home,totals.away,exact.home,exactDef.home);applyGameStats(away,totals.away,totals.home,exact.away,exactDef.away);postGameCondition(home);postGameCondition(away);return finishGame(before,home,away,{home:home.name,away:away.name,hp,ap,winner:win.name,log:log.slice(-160),box:totals,detailed:true,drives});
}
...
function offseasonDepartures(){
 ...
 for(const t of universe.teams){
  ...
  for(const p of t.roster){
   ...
   if(Math.random()<risk/360)portal.push(ensurePortalEntry({p,from:t.name,fromSchoolId:t.id,reason:transferReason(p),enteredSeason:universe.year,wasStarter:(p.stats?.starts||0)>=6}));
   else keep.push(p);
  }
  t.roster=keep;
