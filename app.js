...
function ensureSeasonGoals(t){
 ...
 const add=(id,tier,type,label,target=null)=>goals.push({id,tier,type,label,target,year:universe.year});
 add('wins','Primary','wins',`Win at least ${exp} games`,exp);
 if(rival)add('rivalry','Secondary','rivalry',`Beat ${rival.name}`,rival.id);
 add('recruiting','Secondary','bluechips',`Sign ${blueTarget} blue-chip recruits`,blueTarget);
 if((t.prestige||0)>=82)add('stretch','Stretch','conferenceTitle','Win the conference championship');
 else if((t.prestige||0)>=55)add('stretch','Stretch','top25','Finish in the Top 25',25);
 else add('stretch','Stretch','bowl','Reach bowl eligibility',6);
...
function seasonGoalStatus(t,g,final=false){
 ...
 if(g.type==='wins'){const done=(t.w||0)>=g.target,played=(t.w||0)+(t.l||0);return{state:done?'complete':final||played>=12?'failed':'pending',progress:`${t.w||0}/${g.target} wins`}}
 if(g.type==='rivalry'){const s=t.rivalry?.series,played=s?.lastYear===universe.year;if(!played)return{state:final?'failed':'pending',progress:'Game pending'};return{state:s.lastResult==='W'?'complete':'failed',progress:s.lastResult==='W'?'Won rivalry':'Lost rivalry'}}
 if(g.type==='bluechips'){const n=(universe.recruits||[]).filter(r=>r.committed===t.name&&(r.stars||0)>=4).length;return{state:n>=g.target?'complete':final?'failed':'pending',progress:`${n}/${g.target} blue chips`}}
 if(g.type==='conferenceTitle'){const done=(universe.confChamps||[]).some(x=>x?.name===t.name);return{state:done?'complete':final?'failed':'pending',progress:done?'Conference champion':'Still alive'}}
 if(g.type==='top25'){const done=(t.rank||999)<=25;return{state:done?'complete':final?'failed':'pending',progress:`#${t.rank||'—'} nationally`}}
 if(g.type==='bowl'){const done=(t.w||0)>=6||t.bowlResult?.year===universe.year;return{state:done?'complete':final?'failed':'pending',progress:`${t.w||0}/6 wins`}}
...
function evaluateSeasonGoals(t,final=false){
 ...
 let adjustment=0;
 if(final)for(const r of results){if(r.tier==='Primary')continue;if(r.state==='complete')adjustment+=r.tier==='Stretch'?4:3;else if(r.state==='failed')adjustment-=r.tier==='Stretch'?1:3}
 const completed=results.filter(r=>r.state==='complete').length;
 return{results,completed,total:results.length,adjustment};
}
function seasonGoalsHTML(t){
 const review=evaluateSeasonGoals(t,false);
 return review.results.map(g=>{const icon=g.state==='complete'?'✓':g.state==='failed'?'✕':'•';return `<div class="lineitem"><span><span class="pill">${g.tier}</span> ${icon} ${esc(g.label)}</span><strong>${esc(g.progress)}</strong></div>`}).join('')+`<div class="compact muted" style="margin-top:8px">${review.completed} of ${review.total} goals currently satisfied. Secondary and stretch goals affect the year-end administration review.</div>`;
}
