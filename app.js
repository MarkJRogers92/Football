...
// Opponent scouting / weekly gameplan. Every other Coach's Desk card trades one real thing for
// another; this is the missing per-opponent one. Prep buys a small, game-scoped edge against this
// week's specific opponent. The cost is real only when it can be: schemeFamiliarity already exists
// for a program mid-installation, and prep time spent on an opponent is time not spent installing.
// A fully-installed program pays nothing, which is realistic — it has the staff bandwidth to spare.
const GAMEPLAN_TIERS={scout:{off:1.2,def:1.8,famCost:8},balance:{off:.6,def:.9,famCost:3},standard:{off:0,def:0,famCost:0}};
...
function weeklyGameplanDecision(t){
 ...
 return decisionRecord('WEEKLY_GAMEPLAN',t,null,`Gameplan for ${opp.name}`,
  `${opp.offScheme} / ${opp.defScheme} · ${opp.w}-${opp.l}${installing?'. Extra prep pulls time from installing your new scheme.':''}`,
  [decisionOption('scout',`Full scout`,installing?'A real edge this week, at a real cost to how fast the new scheme goes in.':'A real edge this week, at no real cost — the scheme is already in.'),
   decisionOption('balance','Balanced week','Some edge, smaller cost.',true),
   decisionOption('standard','Standard week','No special prep either way.')],
  {},58);
}
function applyGameplanDecision(t,option){
 ...
 t.gameplan={year:universe.year,week:universe.week,opponent:oppName,prep:option.id};
 if(tier?.famCost){
  for(const side of ['off','def']){
   const tr=schemeTransition(t,side);
