...
function applyGameStats(team,s,opp,exactOffense=null){const q=roleStarter(team,'QB1')||starter(team,'QB');let core=[q,roleStarter(team,'RB1'),roleStarter(team,'X'),roleStarter(team,'Z'),roleStarter(team,'SLOT'),roleStarter(team,'TE1'),roleStarter(team,'LT'),roleStarter(team,'LG'),roleStarter(team,'C1'),roleStarter(team,'RG'),roleStarter(team,'RT'),roleStarter(team,'RUSH'),roleStarter(team,'SETEDGE'),roleStarter(team,'NT'),roleStarter(team,'3TECH'),roleStarter(team,'MIKE'),roleStarter(team,'WILL'),roleStarter(team,'NICKEL'),roleStarter(team,'BCB'),roleStarter(team,'FCB'),roleStarter(team,'FS'),roleStarter(team,'BOXS')].filter(Boolean);const starters=new Set();for(const p of [...new Map(core.map(p=>[p.id,p])).values()]){addGameAppearance(p,true);starters.add(p.id)}
...
function detailedGame(home,away,neutral=false,confOverride=null,context={}){
 ...
 const exact={home:new Map(),away:new Map()};
 function drive(off,def,O,D,scheme,out,label,actors){let pos=25,down=1,toGo=10,plays=0,last=null;const qb=starter(off,'QB'),oc=off.staff.OC,dc=def.staff.DC;while(plays<16){plays++;out.plays++;let pass=scheme.pass+(down===3?.18:0)+(toGo>=8?.09:0)-(toGo<=2?.13:0),pen=Math.random()<.045;if(pen){if(Math.random()<.65){pos=Math.max(1,pos-5);toGo+=5;log.push(`${label}: ${off.name} false start; backed up five.`)}else{pos=Math.min(99,pos+5);toGo=Math.max(1,toGo-5);log.push(`${label}: defensive penalty gives ${off.name} five.`)}continue}
 const isPass=Math.random()<pass;if(isPass){const target=weightedPlayer(receivingShares(off))||starter(off,'WR'),qs=exactOffenseLine(actors,qb),ts=exactOffenseLine(actors,target),pressure=clamp(.17+(D.front-O.ol)*.005+(DEF_SCHEMES[def.defScheme].pressure-70)*.0015+(dc.playCall-70)*.001-(oc.playCall-70)*.0008+(D.prepPressure||0),.08,.48);if(Math.random()<pressure*.28){let loss=gi(3,10);pos=Math.max(1,pos-loss);out.sacksTaken++;qs.sacksTaken++;down++;toGo+=loss;last=null;log.push(`${label}: ${qb.name} sacked for ${loss}.`)}else{out.passAtt++;qs.passAtt++;ts.targets++;let intP=clamp(.027+(D.coverage-O.qb)*.0014+(75-qb.iq)*.0007+(pressure>.30?.009:0),.008,.09);if(Math.random()<intP){out.int++;out.turnovers++;qs.int++;log.push(`${label}: ${qb.name} intercepted targeting ${target.name} near the ${Math.min(99,pos+gi(5,22))}.`);return {points:0,result:'INT',plays}}let comp=clamp(.61+(O.qb-D.coverage)*.004+(qb.technique-70)*.0015+(target.technique-70)*.0007-(pressure>.28?.07:0),.38,.81);if(Math.random()<comp){let y=Math.round(clamp(7+(O.skill-D.coverage)/8+gauss()*6,0,38));if(Math.random()<.055+(qb.speed+target.speed-140)*.0005)y+=gi(12,38);pos+=y;out.passComp++;out.passYds+=y;out.recYds+=y;qs.passComp++;qs.passYds+=y;ts.receptions++;ts.recYds+=y;ts.yac+=Math.round(y*rng(.25,.48));last={type:'pass',qs,ts};if(y>=toGo){out.firstDowns++;down=1;toGo=Math.min(10,100-pos)}else{down++;toGo-=y}log.push(`${label}: ${qb.name} to ${target.name} for ${y}.`)}else{if(Math.random()<clamp((90-target.technique)/250,0,.15))ts.drops++;last=null;down++;log.push(`${label}: ${qb.name} incomplete for ${target.name}.`)}}}
 else{const rusher=weightedPlayer(rushingShares(off))||starter(off,'RB'),rs=exactOffenseLine(actors,rusher);out.rushAtt++;rs.rushAtt++;let runEdge=(O.ol*.38+O.skill*.25+rusher.power*.15+rusher.speed*.12+oc.playCall*.10)-(D.front*.60+DEF_SCHEMES[def.defScheme].run*.20+dc.playCall*.20),y=Math.round(clamp(3.7+runEdge/20+gauss()*3.2,-4,24));if(Math.random()<.045+Math.max(0,rusher.speed-82)*.001)y+=gi(10,32);pos+=y;out.rushYds+=y;rs.rushYds+=y;last={type:'run',rs};if(y>=toGo){out.firstDowns++;down=1;toGo=Math.min(10,100-pos)}else{down++;toGo=Math.max(1,toGo-y)}log.push(`${label}: ${rusher.name} ${rusher===qb?'keeps':'runs'} for ${y}.`)}
 ...
 let hp=0,ap=0;const driveCount=detailedDriveCount(hScheme,aScheme),openingSide=Math.random()<.5?'home':'away',sideCounts={home:0,away:0};for(let i=0;i<driveCount;i++){const side=i%2?(openingSide==='home'?'away':'home'):openingSide,isHome=side==='home',label=`${isHome?'H':'A'}${++sideCounts[side]}`,logStart=log.length,d=drive(isHome?home:away,isHome?away:home,isHome?H:A,isHome?A:H,isHome?hScheme:aScheme,totals[side],label,exact[side]);d.playByPlay=log.slice(logStart).map(x=>x.startsWith(label+': ')?x.slice(label.length+2):x);if(isHome)hp+=d.points;else ap+=d.points;drives.push({side,label,...d})}hp+=homeFieldScoreBonus(home,neutral);if(hp===ap){log.push('Overtime required.');if(Math.random()<.5)hp+=3;else ap+=3}const conf=confOverride??home.conference===away.conference,win=recordGame(home,away,hp,ap,conf,H,A);totals.home.pts=hp;totals.away.pts=ap;applyGameStats(home,totals.home,totals.away,exact.home);applyGameStats(away,totals.away,totals.home,exact.away);postGameCondition(home);postGameCondition(away);return finishGame(before,home,away,{home:home.name,away:away.name,hp,ap,winner:win.name,log:log.slice(-160),box:totals,detailed:true,drives});
}
