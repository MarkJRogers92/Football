...
function offseasonDepartures(){
 ...
 for(const t of universe.teams){
  ...
  const keep=[];
  for(const p of t.roster){
   const risk=transferRisk(p);
   if(gameplayRandom()<risk/360)portal.push(ensurePortalEntry({p,from:t.name,fromSchoolId:t.id,reason:transferReason(p),enteredSeason:universe.year,wasStarter:(p.stats?.starts||0)>=6}));
   else keep.push(p);
  }
  t.roster=keep;
