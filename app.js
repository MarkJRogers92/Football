...
function developmentResult(p,t,phase,before,extra={}){const delta=developmentDelta(before,developmentSnapshot(p));return{id:p.id,name:p.name,pos:p.pos,phase:phase.toUpperCase(),delta:delta.overallDelta,...delta,physical:p.lastPhysicalChange||'',focus:p.trainingFocus||'Balanced',teamFocus:t.trainingFocus||'Balanced',...extra}}
...
function calendarSpringCamp(){
 ...
 if(ensureDevelopmentState().springRun)autosaveAfter('camp'),finishOffseasonPhase('spring','fall',{rosterSize:universe.teams.reduce((n,t)=>n+t.roster.length,0)});
 render();
}
function calendarFallCamp(){
 ...
 if(ensureDevelopmentState().fallRun)autosaveAfter('camp'),finishOffseasonPhase('fall','preseason',{rosterSize:universe.teams.reduce((n,t)=>n+t.roster.length,0)});
 render();
}
