...
function renderMatchupStrategy(){const u=selected(),g=findUserGame();if(!g)return;const opp=T(g.home===u.name?g.away:g.home),m=gameMatchup(u,opp),P=m.teamProfile,O=m.opponentProfile,e=m.teamEdges,oe=m.opponentEdges,plan=teamGameplanFor(u,opp.name),tier=GAMEPLAN_TIERS[plan?.prep]||GAMEPLAN_TIERS.standard,rec=gameplanRecommendation(u,opp,m),home=g.home===u.name?`Home field: about +${homeFieldFor(u).toFixed(1)} points`:'Road game: opponent receives its home-field edge',outU=u.roster.filter(p=>!gameAvailable(p)).length,outO=opp.roster.filter(p=>!gameAvailable(p)).length,row=(label,value)=>`<div class="lineitem"><span>${label}</span><strong class="${value>=3?'good':value<=-3?'bad':''}">${matchupEdgeLabel(value)} (${value>=0?'+':''}${value})</strong></div>`;$('#nextGameCard').innerHTML=`<strong>Week ${universe.week+1}: ${g.home===u.name?'vs':'@'} ${gameEscape(opp.name)}</strong><div class="muted">${gameEscape(u.name)}: O ${grade(P.offense)} / D ${grade(P.defense)} · ${gameEscape(opp.name)}: O ${grade(O.offense)} / D ${grade(O.defense)}</div><div class="small muted">Expected opponent mix: ${Math.round(OFF_SCHEMES[opp.offScheme].pass*100)}% pass · ${home} · unavailable: ${outU} yours / ${outO} theirs</div><div class="small"><strong>Active plan:</strong> ${tier.label}. <strong>Staff recommendation:</strong> ${GAMEPLAN_TIERS[rec.id].label} — ${rec.reason}</div>`;$('#keyMatchups').innerHTML=`${row('Your passing game vs their coverage',e.passGame)}${row('Your run game vs their front',e.runGame)}${row('Your pass protection vs their rush',e.passProtection)}${row('Their passing game vs your coverage',oe.passGame)}${row('Their run game vs your front',oe.runGame)}${row('Their protection vs your rush',oe.passProtection)}<div class="small muted">These edges use the same current profiles consumed at kickoff, including health, wear, scheme, staff and the active gameplan.</div>`}
