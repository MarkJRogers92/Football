...
function simulateUserDetailed(){const g=findUserGame();if(!g)return;if(hasPendingCareerChoice()){setStatus('Choose your next job before advancing the week.');return}if(hasPendingWeeklyDecisions()){setStatus('Resolve the Coach’s Desk decisions before playing this week.');return}recoverWeek();const r=detailedGame(T(g.home),T(g.away),false,g.conf);g.gameId=r.gameId;g.played=true;g.score=[r.ap,r.hp];g.winner=r.winner;g.detailed=true;universe.lastDetailedGame={...r,season:universe.year,week:g.week};universe.latest=[r];ranked();render()}
