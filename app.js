...
const GAMEPLAN_TIERS={scout:{off:1.2,def:1.8,famCost:8,wearCost:3},balance:{off:.6,def:.9,famCost:3,wearCost:1},standard:{off:0,def:0,famCost:0,wearCost:0}};
...
function weeklyGameplanDecision(t){
 ...
 return decisionRecord('WEEKLY_GAMEPLAN',t,null,`Gameplan for ${opp.name}`,
  `${opp.offScheme} / ${opp.defScheme} · ${opp.w}-${opp.l}. Extra prep pulls reps from your key starters' rest${installing?' and from installing your new scheme':''}.`,
  [decisionOption('scout',`Full scout`,`The biggest edge this week — costs your starters real wear${installing?' and slows the new scheme going in':''}.`),
   decisionOption('balance','Balanced week','A smaller edge for a smaller wear cost.',true),
   decisionOption('standard','Standard week','No extra prep, no extra cost.')],
  {},58);
}
function applyGameplanDecision(t,option){
 ...
 if(tier?.wearCost){
  for(const p of importantStarters(t).slice(0,5))p.wear=clamp((p.wear||0)+tier.wearCost,0,100);
 }
}
...
function saveTitlePreferences(){const p={motion:$('#titleMotion').checked,watchSpeed:$('#titleWatchSpeed').value};try{localStorage.setItem('dynastyLabPreferences',JSON.stringify(p))}catch{}applyTitlePreferences();setTitleStatus('Options saved on this device.');showTitlePanel()}
...
$$('.tabs button').forEach(b=>b.onclick=()=>{$$('.tabs button').forEach(x=>x.classList.remove('active'));$$('.tab').forEach(x=>x.classList.remove('active'));b.classList.add('active');$('#'+b.dataset.tab).classList.add('active');setActiveTab(b.dataset.tab)});
 ...
 $('#newUniverse').onclick=()=>{if(confirm('Return to the title screen? Save first if you want to preserve this session.'))showTitleScreen()};
 ...
 $('#titleStart').onclick=startTitleDynasty;$('#titleLoadBrowser').onclick=continueTitleDynasty;$('#titleImport').onclick=()=>$('#importFile').click();$('#titleSaveOptions').onclick=saveTitlePreferences;$$('[data-title-back]').forEach(b=>b.onclick=()=>showTitlePanel());
}
