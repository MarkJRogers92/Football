...
function gameAvailable(p){return !!p&&(p.injuryWeeks||0)===0&&!p.redshirtActive}
...
function roleStarter(t,id){let a=rolePlayers(t,id);return a.find(gameAvailable)||a[0]||null}
...
function coachOpeningHubItems(t){if(!t)return[];return (universe.openings||[]).filter(o=>o.schoolId===t.id&&o.status==='OPEN').map(o=>({type:'action',tab:'staff',kicker:'COACHING SEARCH',main:`${o.role} opening`,sub:'Interview and hire from the candidate market on the Staff tab.'}))}
function promiseHubItems(t){return (universe.events||[]).filter(e=>e.type==='PROMISE_BROKEN'&&e.schoolIds.includes(t?.id)&&e.season>=universe.year-1).slice(-3).reverse().map(e=>({type:'bad-news',tab:'roster',player:e.playerIds[0],kicker:'PROMISE BROKEN',main:e.summary,sub:e.metadata.result}))}
...
function beginGame(home,away,neutral,context={}){
 const side=t=>({id:t.id,name:t.name,rank:t.rank,record:`${t.w}-${t.l}`,players:new Map(t.roster.map(p=>[p.id,{stats:{...p.stats},injuries:p.injuryHistory?.length||0}]))});
 return {season:universe.year,week:context.week??universe.week+1,phase:universe.phase,label:context.label||'Regular season',venue:neutral?'Neutral site':`${home.name} · ${home.city||''}`,home:side(home),away:side(away)};
}
...
function showGameCenter(id,tab='Summary'){
 ...
 $('#gameDialogName').textContent=`${g.away.name} ${g.score.away} — ${g.score.home} ${g.home.name}`;
 $('#gameDialogMeta').textContent=`FINAL · ${g.season} · Week ${g.week} · ${g.label} · ${g.venue}`;
 ...
 let html;if(tab==='Box Score')html=gameBoxHTML(g);else if(tab==='Drives')html=g.drives?.length?gameTable(['Drive','Offense','Plays','Result','Points'],g.drives.map(d=>[d.label,g[d.side].name,d.plays,d.result,d.points]))+'<p class="muted">Drive clock and yardage are not recorded. Drives start at the 25 in this engine.</p>':'<p>No drive detail recorded. Use Box Score for the permanent statistics.</p>';else if(tab==='Play-by-Play'){const d=universe.lastDetailedGame;html=d?.gameId===id&&d.log?.length?'<p class="muted">Retained simulation log (last 160 entries). The drive model names primary players; box scores allocate production across the rotation. Full play logs are temporary; box scores and drive summaries are permanent.</p>'+d.log.map(x=>`<div class="playline">${gameEscape(x)}</div>`).join(''):'<p>Play-by-play is unavailable for this archived game. Box scores and recorded drive summaries remain permanent.</p>'}else html=gameSummaryHTML(g);
...
function buildWeeklyHub(beforeRank=selected()?.rank||null){const u=selected(),items=[...pulledOfferHubItems(u),...coachOpeningHubItems(u),...coachFalloutHubItems(u),...promiseHubItems(u)],res=userWeekResult(u);if(res)items.push({type:res.win?'good-news':'bad-news',tab:'season',kicker:'FINAL',gameId:res.gameId,main:`${res.win?'Win':'Loss'} ${res.userPts}–${res.oppPts} ${res.loc} ${res.opp}`,sub:`${u.w}-${u.l} overall · ${u.cw}-${u.cl} conference`});if(beforeRank&&u.rank!==beforeRank)items.push({type:u.rank<beforeRank?'good-news':'alert',tab:'season',kicker:'RANKINGS',main:`${u.rank<beforeRank?'Up':'Down'} to #${u.rank}`,sub:`Previous ranking: #${beforeRank}`});let inj=u.roster.filter(p=>(p.injuryHistory||[]).some(x=>x.year===universe.year&&x.week===universe.week)).slice(0,2);for(const p of inj)items.push({type:'bad-news',tab:'roster',player:p.id,kicker:'MEDICAL',main:`${p.name}: ${p.injury}`,sub:`Expected absence: ${p.injuryWeeks} week${p.injuryWeeks===1?'':'s'}`});let commits=universe.recruits.filter(r=>r.committed===u.name&&r.commitWeek===universe.week).slice(0,3);for(const r of commits)items.push({type:'good-news',tab:'recruiting',recruit:r.id,kicker:'COMMITMENT',main:`${'★'.repeat(r.stars)} ${r.pos} ${r.name}`,sub:`#${r.nationalRank} nationally · ${r.homeCity}, ${r.homeState}`});let next=findUserGame();if(next){let opp=T(next.home===u.name?next.away:next.home);items.push({type:'action',tab:'gamelab',kicker:'NEXT UP',main:`Week ${universe.week+1}: ${next.home===u.name?'vs':'@'} ${opp.name}`,sub:`${opp.w}-${opp.l} · #${opp.rank<=25?opp.rank:'—'} · ${opp.offScheme}`})}let risk=u.roster.map(p=>({p,r:transferRisk(p)})).sort((a,b)=>b.r-a.r)[0];if(risk?.r>=42)items.push({type:'alert',tab:'roster',player:risk.p.id,kicker:'LOCKER ROOM',main:`${risk.p.name} transfer risk: ${Math.round(risk.r)}`,sub:`${risk.p.role} · morale ${risk.p.morale}`});for(const e of (universe.decommitLog||[]).filter(e=>e.year===universe.year&&e.week===universe.week&&(e.from===u.name||e.to===u.name)).slice(0,3))items.push(e.from===u.name?{type:'bad-news',tab:'recruiting',recruit:e.id,kicker:'DECOMMIT',main:`${'★'.repeat(e.stars)} ${e.pos} ${e.name}`,sub:e.to?`Flipped to ${e.to}`:'Reopened his recruitment'}:{type:'good-news',tab:'recruiting',recruit:e.id,kicker:'FLIP',main:`${'★'.repeat(e.stars)} ${e.pos} ${e.name}`,sub:`Flipped from ${e.from}`});let waver=universe.recruits.filter(r=>r.committed===u.name&&r.challenger&&r.pressure>0).sort((a,b)=>b.pressure-a.pressure)[0];if(waver)items.push({type:'alert',tab:'recruiting',recruit:waver.id,kicker:'WAVERING',main:`${waver.name} hearing from ${waver.challenger}`,sub:`${'★'.repeat(waver.stars)} ${waver.pos} commit · target, visit or promise to hold him`});let hot=universe.recruits.filter(r=>r.targeted&&!r.committed).sort((a,b)=>b.interest-a.interest)[0];if(hot)items.push({type:'action',tab:'recruiting',recruit:hot.id,kicker:'TOP TARGET',main:`${hot.name} · ${hot.interest}%`,sub:`#${hot.nationalRank} ${hot.pos} · ${hot.trend>0?'↑ '+hot.trend:hot.trend<0?'↓ '+Math.abs(hot.trend):'steady'}`});universe.weeklyHub=items.slice(0,9)}

function simulateUserDetailed(){const g=findUserGame();if(!g)return;recoverWeek();const r=detailedGame(T(g.home),T(g.away),false,g.conf);g.gameId=r.gameId;g.played=true;g.score=[r.ap,r.hp];g.winner=r.winner;g.detailed=true;universe.lastDetailedGame=r;universe.latest=[r];ranked();render()}
function simWeek(){if(universe.phase!=='regular'||universe.week>=12)return;recoverWeek();const u=selected(),beforeRank=u.rank;let results=[];for(const g of universe.schedule[universe.week]){if(g.played)continue;let r=gameSim(T(g.home),T(g.away),false,g.conf);g.gameId=r.gameId;g.played=true;g.score=[r.ap,r.hp];g.winner=r.winner;results.push(r)}universe.week++;if(results.length)universe.latest=results;ranked();advanceRecruiting();buildWeeklyHub(beforeRank);if(universe.week===12)universe.phase='confReady';render()}
...
function pulledOfferHubItems(t){
 ...
 return (universe.events||[]).filter(e=>e.type==='OFFER_PULLED'&&e.schoolIds.includes(t.id)&&e.season===universe.year).slice(-2).reverse()
  .map(e=>({type:'bad-news',tab:'recruiting',kicker:'OFFER PULLED',main:e.summary,
...
function transferRisk(p){return clamp((58-p.morale)+(p.role==='Development'?12:0)+(p.role==='Redshirt candidate'?8:0)+promisePenalty(p)+coachTransferPressure(p)+schemeFitPressure(p)+(p.injuryWeeks>3?3:0),0,100)}
...
function setActiveTab(id){activeTab=id;renderActiveTab()}
...
function render(){const order=ranked();const u=selected();if(!u)return;const p=profiles(u);$('#teamName').textContent=u.name;$('#teamMeta').textContent=`${u.conference} · ${u.offScheme} / ${u.defScheme} · Prestige ${u.prestige} · Ceiling ${u.program_ceiling}`;$('#teamMetrics').innerHTML=[['Offense',grade(p.offense)],['Defense',grade(p.defense)],['Scheme Fit',grade((p.offFit+p.defFit)/2)],['Development',grade(u.development)]].map(x=>`<div class="metric"><div class="small muted">${x[0]}</div><div class="v">${x[1]}</div></div>`).join('');$('#recordBig').textContent=`${u.w}–${u.l}`;$('#rankLine').textContent=u.rank<=25?`#${u.rank} nationally`:`#${u.rank} nationally · outside the top 25`;$('#weekLine').textContent=`${universe.year} · Week ${universe.week}`;$('#top15').innerHTML=order.slice(0,15).map((t,i)=>`<div class="rankrow"><div><span class="rank">${i+1}.</span> ${t.name}</div><div>${t.w}-${t.l}</div></div>`).join('');const leaders=u.roster.slice().sort((a,b)=>b.perceived-a.perceived).slice(0,5);$('#snapshot').innerHTML=`<div class="lineitem"><span>Roster</span><strong>${u.roster.length}</strong></div><div class="lineitem"><span>Protected redshirts</span><strong>${u.roster.filter(p=>p.redshirtActive).length}</strong></div><div class="lineitem"><span>Commits</span><strong>${u.commits.length}</strong></div><div class="lineitem"><span>Head Coach</span><strong>${u.staff.HC.name}</strong></div><div class="lineitem"><span>PF / PA</span><strong>${u.pf} / ${u.pa}</strong></div>`+leaders.map(x=>`<div class="lineitem compact"><span>${x.pos} ${x.name}</span><span>${playerGrade(x)} · ${x.style}</span></div>`).join('');renderWeeklyPlan();renderWeeklyHub();renderActiveTab();$('#simConf').disabled=universe.phase!=='confReady';$('#simPlayoff').disabled=universe.phase!=='playoffReady';let devState=ensureDevelopmentState();$('#runOffseason').disabled=universe.phase!=='complete'||!devState.springRun||!devState.fallRun;$('#simDetailedGame').disabled=!findUserGame()}
...
function showPlayerProfile(id){let f=findPlayer(id);if(!f&&archiveIsDeferred()){const current=universe;return ensureArchiveLoaded().then(()=>{if(universe===current)showPlayerProfile(id)}).catch(e=>{if(universe===current)setStatus(e.message)})}if(!f)return;const p=f.p,t=f.team;{const ph=$('#playerDialogPortrait');if(ph){ph.innerHTML=portraitTag(p,192,'profile');}}
 $('#playerDialogName').textContent=p.name;$('#playerDialogMeta').textContent=`${p.pos} · ${eligibilityLabel(p)} · ${heightStr(p.height)} / ${p.weight} · ${p.style}${t?` · ${t.name}`:''}`;$('#playerDialogKicker').textContent=f.portal?'TRANSFER PORTAL':f.active?'ACTIVE PLAYER':'PLAYER ARCHIVE';if(f.active&&t)ensurePlayerDevelopment(p,t);let attrs=[['Current',f.active&&t?scoutRangeText(p,t,false):grade(p.perceived)],['Upside',f.active&&t?scoutRangeText(p,t,true):grade(p.perceivedUpside)],['Scout Confidence',f.active?`${Math.round(p.scoutConfidence||50)}%`:'Final'],['Speed',grade(p.speed)],['Power',grade(p.power)],['Technique',grade(p.technique)],['IQ',grade(p.iq)],['Health',p.health??100]];let injury=(p.injuryWeeks||0)>0?`<div class="injury-note"><strong>${p.injury}</strong> · expected ${p.injuryWeeks} more week${p.injuryWeeks===1?'':'s'}.</div>`:'';let hist=(p.seasonHistory||[]).slice().reverse().map(s=>`<div class="timeline-row"><strong>${s.year} · ${s.team}</strong><div class="muted">${s.eligibility||s.pos} · ${s.pos} · ${statLine(s)}</div></div>`).join('')||'<div class="muted">No completed college seasons archived yet.</div>',aw=(p.awards||[]).slice().reverse().map(a=>`<div class="lineitem"><span>${a.year}</span><strong>${a.name}</strong></div>`).join('')||'<div class="muted">No major awards.</div>',draft=p.draftResult?`<div class="lineitem"><span>${p.draftResult.year} Draft</span><strong>${p.draftResult.label}</strong></div>`:'';$('#playerDialogBody').innerHTML=`${injury}<div class="profile-grid">${attrs.map(x=>`<div class="profile-stat"><div class="small muted">${x[0]}</div><div class="v">${x[1]}</div></div>`).join('')}</div><div class="two-col"><div><h3>Current Season</h3><div class="lineitem"><span>Production</span><strong>${statLine(p.stats)}</strong></div><div class="lineitem"><span>Eligibility</span><strong>${eligibilityLabel(p)}</strong></div><div class="lineitem"><span>Redshirt</span><strong>${p.redshirtUsed?`Used${p.redshirtSeason?' in '+p.redshirtSeason:''}`:p.redshirtActive?'Protected this year':'Available'}</strong></div><div class="lineitem"><span>Origin</span><strong>${p.origin||'—'}</strong></div><div class="lineitem"><span>Promise</span><strong>${p.promise||'None'}</strong></div><div class="lineitem"><span>Wear</span><strong>${p.wear||0}/100</strong></div><div class="lineitem"><span>Training focus</span><strong>${p.trainingFocus||'Balanced'}</strong></div><div class="lineitem"><span>Position familiarity</span><strong>${familiarity(p,p.pos)}%</strong></div><div class="lineitem"><span>Staff development note</span><strong>${f.active&&t?devStaffNote(p,t):'Career complete'}</strong></div></div><div><h3>Career</h3><div class="lineitem"><span>Career production</span><strong>${statLine(p.career)}</strong></div><div class="lineitem"><span>Injuries recorded</span><strong>${p.injuryHistory?.length||0}</strong></div><div class="lineitem"><span>Status</span><strong>${f.active?healthText(p):(f.portal?'Awaiting a destination':p.exitReason||'Career complete')}</strong></div>${draft}<h3 style="margin-top:12px">Honors</h3>${aw}</div></div>${promiseHTML(p)}${transferHistoryHTML(p)}<h3 style="margin-top:14px">Season Timeline</h3><div class="timeline">${hist}</div>`;let d=$('#playerDialog');if(d.showModal)d.showModal();else d.setAttribute('open','');paintPortraits(d)}
...
function normalizeUniverse(){universe.gameArchive??=[];universe.gameArchiveVersion??=1;universe.gameCounter=universe.gameArchive.reduce((n,g)=>Math.max(n,Number(g.id.split('_').pop())||0),universe.gameCounter||0);universe.version=APP_VERSION;IDX.teams=null;IDX.players=null;universe.movementLog??=[];universe.lastDetailedGame??=null;universe.playerArchive??=[];universe.recoveredWeek??=-1;universe.weeklyHub??=[];universe.highSchools??=generateHighSchools();universe.awards??={};universe.records??={nationalSeason:{},nationalCareer:{}};universe.records.nationalSeason??={};universe.records.nationalCareer??={};universe.draftHistory??={};universe.recruitClassCounts??={};universe.campHistory??={};universe.developmentState??={year:universe.year,springRun:false,fallRun:false,springReport:[],fallReport:[],battles:[]};if(!Object.keys(universe.recruitClassCounts).length)for(const r of universe.recruits||[])if(r.committed)universe.recruitClassCounts[r.committed]=(universe.recruitClassCounts[r.committed]||0)+1;universe.teams.forEach(t=>{let base=schools.find(s=>s.name===t.name);if(base){t.city??=base.city;t.state??=base.state;t.lat??=base.lat;t.lon??=base.lon}t.staff??=generateStaff(t);for(const c of Object.values(t.staff)){c.contractYears??=gi(1,4);c.salary??=1.2}t.offScheme??=pick(Object.keys(OFF_SCHEMES));t.defScheme??=pick(Object.keys(DEF_SCHEMES));t.nickname??='';t.commits??=[];t.pipelines??=makePipelines(t);t.records??={};ensureSchoolColors(t);ensureTeamDevelopment(t);t.roster.forEach(p=>{p.speed??=clamp(p.trueNow+gi(-8,8),25,99);p.power??=clamp(p.trueNow+gi(-8,8),25,99);p.technique??=clamp(p.trueNow+gi(-8,8),25,99);p.iq??=clamp(p.trueNow+gi(-10,10),20,99);p.composure??=65;p.durability??=70;p.versatility??=60;p.health??=100;p.wear??=0;p.injury??=null;p.injuryWeeks??=0;p.injuryHistory??=[];p.seasonHistory??=[];p.awards??=[];p.draftResult??=null;p.promise??=null;p.promiseBaseline??=p.perceived;p.eligibilityUsed??=({FR:0,SO:1,JR:2,SR:3}[p.year]??0);p.redshirtUsed??=false;p.redshirtActive??=false;p.redshirtSeason??=null;p.stats={...newStats(),...(p.stats||{})};p.career={...newStats(),...(p.career||{})};p.year=CLASS_NAMES[Math.min(3,eligibilityBase(p))]||'SR';ensurePlayerDevelopment(p,t)});ensureDepth(t);ensureRoleDepth(t)});universe.playerArchive.forEach(p=>{p.awards??=[];p.seasonHistory??=[];p.coachRelationships??={};p.primaryRecruiterCoachId??=p.recruitingMemory?.primaryRecruiterCoachId||p.recruitingMemory?.recruiterCoachId||null;p.career={...newStats(),...(p.career||{})};p.stats={...newStats(),...(p.stats||{})};p.eligibilityUsed??=3;p.redshirtUsed??=false;p.redshirtActive=false});if(Array.isArray(universe.schedule)){universe.teams.forEach(t=>t.schedule=[]);universe.schedule.flat().forEach(g=>{T(g.home)?.schedule.push(g);T(g.away)?.schedule.push(g)})}
 normalizePromiseState();for(const r of universe.recruits||[])normalizeRecruitGeography(r,universe.highSchools);assignRecruitRanks(universe.recruits||[]);if(!universe.weeklyHub.length)buildPreseasonHub();rebuildIndexes()}
...
async function storageOperation(action){
  ...
  finally{controls.forEach((el,i)=>el.disabled=disabled[i]);storageBusy=false;render()}
}
...
function loadBrowser(){return storageOperation(async()=>{
  ...
  if(!d){setStatus('No browser save found.');return}
...
loadSchools().then(()=>{initUniverse();refreshTeamOptions('Chicago Metropolitan');POS.forEach(p=>{let o=document.createElement('option');o.value=p;o.textContent=p;$('#positionFilter').appendChild(o)});allConfs().forEach(c=>{let o=document.createElement('option');o.value=c;o.textContent=c;$('#editConference').appendChild(o)});bind();render();setStatus(`v${APP_VERSION} ready. Export or Save to preserve a dynasty.`);}).catch(e=>{$('#app').innerHTML='<div style="padding:30px">Could not initialize Dynasty Lab. '+e.message+'</div>'});
})();
