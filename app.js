...
function offseasonEnrollment(){
 ...
 finishOffseasonPhase('signing','portal',teams);
 return true;
}
function offseasonPortal(){
 ...
 finishOffseasonPhase('portal','spring',{placed:portal.length-universe.transferPortal.length,pending:universe.transferPortal.length});
 return true;
}
...
function calendarSpringCamp(){
 ...
 if(ensureDevelopmentState().springRun)finishOffseasonPhase('spring','fall',{rosterSize:universe.teams.reduce((n,t)=>n+t.roster.length,0)});
 render();
}
function calendarFallCamp(){
 ...
 if(ensureDevelopmentState().fallRun)finishOffseasonPhase('fall','preseason',{rosterSize:universe.teams.reduce((n,t)=>n+t.roster.length,0)});
 render();
}
