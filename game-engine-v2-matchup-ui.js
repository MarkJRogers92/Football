...
function v2MatchupPlanHTML(team,opp,matchup){
  ...
  return`<div class="v2-matchup-plan"><div><span>Active plan</span><strong>${gameEscape(tier.label)}</strong></div><div><span>Staff recommendation</span><strong>${gameEscape(recommended.label)}</strong><small>${gameEscape(rec.reason)}</small></div><div class="v2-matchup-plan-action">${plan?'<span class="pill good">Plan locked for kickoff</span>':'<button type="button" data-v2-open-coachs-desk>Set plan in Coach’s Desk</button>'}</div></div>`
}
function v2MatchupStakes(team,opp){
  const rival=rivalOf(team),isRival=rival?.name===opp.name,sameConference=team.conference===opp.conference;
  if(isRival)return`${team.rivalry?.trophy||'Rivalry game'} · ${rivalrySeriesText(team)}`;if(sameConference)return`${team.conference} Conference game`;return'Nonconference game'
}
...
function ensureV2PlanReceiptHost(host){let receipt=document.querySelector('#v2PlanReceipt');if(!receipt&&host){receipt=document.createElement('div');receipt.id='v2PlanReceipt';host.insertAdjacentElement('afterend',receipt)}return receipt}
function v2MatchupReceiptHTML(){
  ...
  const tone=receipt.verdict==='Worked'?'good':receipt.verdict==='Missed'?'bad':receipt.verdict==='Baseline'?'':'warn';
...
function renderV2MatchupIntelligence(){
  const host=ensureV2MatchupHost();if(!host)return;const receipt=ensureV2PlanReceiptHost(host),team=selected(),g=findUserGame();if(receipt)receipt.innerHTML=v2MatchupReceiptHTML();
  if(!team||!g){host.hidden=true;return}
...
if(globalThis.__DL_TEST__)globalThis.__DL_TEST__.v2MatchupSummary=()=>{const h=document.querySelector('#v2MatchupIntelligence'),r=document.querySelector('#v2PlanReceipt');return{matchup:h?{visible:!h.hidden,text:h.textContent}:null,receipt:r?.textContent||''}};
